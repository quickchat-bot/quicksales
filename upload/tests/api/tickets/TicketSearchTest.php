 ...
 * @group ticketsearch
 */
class TicketSearchTest extends BaseApiTestCase
{
    private static $_id;
    ...
    {
        $response = $this->getResponse('/Tickets/TicketSearch', 'POST', [
            'query' => 'ticket contents',
            'contents' => 1,
            'phrase' => 1,
        ]);
        $list = $this->getArrayFromResponse($response);
        $this->assertEquals('Custom Subject', $list['ticket']['subject']);
    }
}
