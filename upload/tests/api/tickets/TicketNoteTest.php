 ...
 * @group ticketnote
 */
class TicketNoteTest extends BaseApiTestCase
{
    private static $_id;
    ...
    public static function tearDownAfterClass()
    {
        if (isset(static::$_id)) {
            try {
                SWIFT_TicketNote::DeleteList([static::$_id]); // cleanup
            } catch (\Exception $ex) {
            }
        }
    ...
    {
        $response = $this->getResponse('/Tickets/TicketNote', 'POST', [
            'contents' => 'Custom',
            'ticketid' => static::$_ticketId,
            'staffid' => static::$_staffId,
        ]);
        $list = $this->getArrayFromResponse($response, 0);
        $this->assertEquals(static::$_ticketId, $list['note']['@attributes']['ticketid']);
        static::$_id = $list['note']['@attributes']['id'];
    }
    ...
    {
        $response = $this->getResponse('/Tickets/TicketNote/ListAll/' . static::$_ticketId);
        $list = $this->getArrayFromResponse($response, 0);
        $id = static::$_id;
        if (!isset($list['note']['@attributes']['ticketid'])) {
            $list = array_values(array_filter($list['note']['@attributes'], function ($ug) use ($id) {
                return $ug['id'] === $id;
            }));
        } else {
            $list = [$list['note']['@attributes']];
        }
    ...
    {
        $response = $this->getResponse('/Tickets/TicketNote/' . static::$_ticketId . '/' . self::$_id);
        $list = $this->getArrayFromResponse($response, 0);
        $this->assertEquals(static::$_ticketId, $list['note']['@attributes']['ticketid']);
    }
    ...
    {
        $response = $this->getResponse('/Tickets/TicketNote/' . static::$_ticketId . '/' . self::$_id, 'DELETE');
