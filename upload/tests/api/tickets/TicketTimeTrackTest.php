 ...
 * @group tickets
 * @group tickettimetrack
 */
class TicketTimeTrackTest extends BaseApiTestCase
{
    private static $_id;
    ...
    public static function tearDownAfterClass()
    {
        if (isset(static::$_id)) {
            try {
                SWIFT_TicketTimeTrack::DeleteList([static::$_id]); // cleanup
            } catch (\Exception $ex) {
            }
        }
    ...
    {
        $response = $this->getResponse('/Tickets/TicketTimeTrack', 'POST', [
            'contents' => 'Custom',
        ...
        $list = $this->getArrayFromResponse($response, 0);
        $this->assertEquals(static::$_ticketId, $list['timetrack']['@attributes']['ticketid']);
        static::$_id = $list['timetrack']['@attributes']['id'];
    }
    ...
    {
        $response = $this->getResponse('/Tickets/TicketTimeTrack/ListAll/' . static::$_ticketId);
        $list = $this->getArrayFromResponse($response, 0);
        $id = static::$_id;
        if (!isset($list['timetrack']['@attributes']['ticketid'])) {
            $list = array_values(array_filter($list['timetrack']['@attributes'], function ($ug) use ($id) {
                return $ug['id'] === $id;
            }));
        } else {
            $list = [$list['timetrack']['@attributes']];
        }
    ...
    {
        $response = $this->getResponse('/Tickets/TicketTimeTrack/' . static::$_ticketId . '/' . self::$_id);
        $list = $this->getArrayFromResponse($response, 0);
        $this->assertEquals(static::$_ticketId, $list['timetrack']['@attributes']['ticketid']);
    }
    ...
    {
        $response = $this->getResponse('/Tickets/TicketTimeTrack/' . static::$_ticketId . '/' . self::$_id, 'DELETE');
