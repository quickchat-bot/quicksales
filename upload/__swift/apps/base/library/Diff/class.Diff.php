...
Modified by Werner Garcia on June, 2019
...
class Diff
{
    ...
    protected static $_leftCount = 0;
    protected static $_rightCount = 0;
    ...
    public static function toTable($diff, $indentation = '', $separator = '<br>')
    {
        ...
        // initialise the HTML
        $html = $indentation . "<table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" class=\"gridcontentborder diffcontainer\">\n";

        // loop over the lines in the diff
        $index = 0;
        static::$_leftCount = 1;
        static::$_rightCount = 1;
        while ($index < count($diff)) {
            ...
            switch ($diff[$index][1]) {
                ...
                case self::UNMODIFIED:
                    $leftCell  =
                        self::getCellContent(
                            $diff, $indentation, '', $index, self::UNMODIFIED);
                    $rightCell = $leftCell;
                    $leftCell = preg_replace_callback('/MYNUMBER/', function ($matches) {
                        return static::$_leftCount++;
                    }, $leftCell);
                    $rightCell = preg_replace_callback('/MYNUMBER/', function ($matches) {
                        return static::$_rightCount++;
                    }, $rightCell);
                    break;
                ...
                case self::DELETED:
                    $leftCell  =
                        self::getCellContent(
                            $diff, $indentation, '', $index, self::DELETED, static::$_leftCount);
                    $rightCell =
                        self::getCellContent(
                            $diff, $indentation, '', $index, self::INSERTED, static::$_rightCount);
                    break;
            ...
            if ($leftCell === '') {
                $leftText = $leftCell === $rightCell
                    ? $leftCell
                    : str_replace('CHANGEME', 'difflineempty', $leftCell);
            } else {
                $leftText = $leftCell === $rightCell
                    ? $leftCell
                    : str_replace('CHANGEME',
                        'difflinedeleted', $leftCell);
            }
            if ($rightCell === '') {
                $rightText = $leftCell === $rightCell
                    ? $rightCell
                    : str_replace('CHANGEME', 'difflineempty', $rightCell);
            } else {
                $rightText = $leftCell === $rightCell
                    ? $rightCell
                    : str_replace('CHANGEME',
                        'difflineadded', $rightCell);
            }
            $html .=
                $indentation
                . "  <tr>\n"
    ...
    private static function getCellContent(
        ...
        $class = ($type === self::UNMODIFIED)? ' class="difflineunchanged"' : ' class="CHANGEME"';
        while ($index < count($diff) && $diff[$index][1] == $type) {
            $str = $diff[$index][0] ? htmlspecialchars($diff[$index][0]): '<img src="' . SWIFT::Get('themepath') . 'images/space.gif"/>';
            $html             .=
                '<div class="difflinerow"><div class="diffnumber">'.($line ?? 'MYNUMBER').'.</div><div' . $class . '>'
                . $str
                . '</div></div>'
                . $separator;
            $index++;
