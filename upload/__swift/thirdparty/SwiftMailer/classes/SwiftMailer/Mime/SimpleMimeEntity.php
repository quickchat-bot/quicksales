...
class SwiftMailer_Mime_SimpleMimeEntity implements SwiftMailer_Mime_MimeEntity
{
     ...
     */
    public function __construct(SwiftMailer_Mime_HeaderSet $headers, SwiftMailer_Mime_ContentEncoder $encoder, SwiftMailer_KeyCache $cache, SwiftMailer_Mime_Grammar $grammar)
    {
    ...
    {
        return $this->_body instanceof SwiftMailer_OutputByteStream ? $this->_readStream($this->_body) : $this->_body;
    }
    ...
    protected function _bodyToString()
    {
        ...
        if (isset($this->_body) && empty($this->_immediateChildren)) {
            if ($this->_cache->hasKey($this->_cacheKey, 'body')) {
                ...
                $this->_cache->setString($this->_cacheKey, 'body', $body, SwiftMailer_KeyCache::MODE_WRITE);
            }
            $string .= $body;
        }
    ...
    /**
     * Write this entire entity to a {@see SwiftMailer_InputByteStream}.
     *
     ...
     * @param SwiftMailer_InputByteStream
     */
    protected function _bodyToByteStream(SwiftMailer_InputByteStream $is)
    {
        if (empty($this->_immediateChildren)) {
            if (isset($this->_body)) {
                if ($this->_cache->hasKey($this->_cacheKey, 'body')) {
    ...
    {
        if ($this->_cache instanceof SwiftMailer_KeyCache) {
    ...
    private function _assertValidId($id)
    {
        if (!preg_match('/^'.$this->_grammar->getDefinition('id-left').'@'.$this->_grammar->getDefinition('id-right').'$/D', $id)) {
            throw new SwiftMailer_RfcComplianceException('Invalid ID given <'.$id.'>');
        }
    }
