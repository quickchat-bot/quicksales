 ...
 * @author Chris Corbyn
 */
abstract class SwiftMailer_Transport_AbstractSmtpTransport implements SwiftMailer_Transport
{
     ...
     */
    public function __construct(SwiftMailer_Transport_IoBuffer $buf, SwiftMailer_Events_EventDispatcher $dispatcher)
    {
    ...
    protected function _getFullResponse($seq)
    {
        ...
        try {
            ...
            $this->_throwException($e);
        } catch (SwiftMailer_IoException $e) {
            $this->_throwException(
                new SwiftMailer_TransportException(
                    $e->getMessage())
                );
    ...
    private function _sendTo(SwiftMailer_Mime_Message $message, $reversePath, array $to, array &$failedRecipients)
    {
        if (empty($to)) {
            return 0;
        }
    ...
    private function _sendBcc(SwiftMailer_Mime_Message $message, $reversePath, array $bcc, array &$failedRecipients)
    {
        $sent = 0;
    ...
    private function _lookupHostname()
    {
        if (!empty($_SERVER['SERVER_NAME']) && $this->_isFqdn($_SERVER['SERVER_NAME'])) {
             ...
             *
             * Comments: Adding [IPv6:] prefix in case of IPv6
             */
