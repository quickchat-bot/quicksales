 ...
 */
class SwiftMailer_Signers_DomainKeySigner implements SwiftMailer_Signers_HeaderSigner
{
    /**
     * PrivateKey.
     *
            ...
            switch ($string[$i]) {
                ...
                case "\n":
                    if ($this->_bodyCanonLastChar == "\r") {
                        ...
                        // Wooops Error
                        throw new SwiftMailer_SwiftMailerException('Invalid new line sequence in mail found \n without preceding \r');
                    }
                    break;
                case ' ':
                case "\t":
    ...
    private function _getEncryptedHash()
    {
        ...
        if (!$pkeyId) {
            throw new SwiftMailer_SwiftMailerException('Unable to load DomainKey Private Key ['.openssl_error_string().']');
        }
        if (openssl_sign($this->_canonData, $signature, $pkeyId, OPENSSL_ALGO_SHA1)) {
            return $signature;
        }
        throw new SwiftMailer_SwiftMailerException('Unable to sign DomainKey Hash  ['.openssl_error_string().']');
    }
}
