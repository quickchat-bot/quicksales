 ...
 */
class SwiftMailer_Signers_SMimeSigner implements SwiftMailer_Signers_BodySigner
{
    ...
    /**
     * @var SwiftMailer_StreamFilters_StringReplacementFilterFactory
     */
    protected $replacementFactory;

    /**
     * @var SwiftMailer_Mime_HeaderFactory
     */
    ...
    public function __construct($signCertificate = null, $signPrivateKey = null, $encryptCertificate = null)
    {
        ...
        $this->replacementFactory = SwiftMailer_DependencyContainer::getInstance()
    ...
    /**
     * Returns an new SwiftMailer_Signers_SMimeSigner instance.
     *
    ...
    /**
     * Change the SwiftMailer_Message to apply the signing.
     *
    ...
    public function signMessage(SwiftMailer_Message $message)
    {
        ...
        $messageStream = new SwiftMailer_ByteStream_TemporaryFileByteStream();
        $this->toSMimeByteStream($messageStream, $message);
        $message->setEncoder(SwiftMailer_DependencyContainer::getInstance()->lookup('mime.rawcontentencoder'));
    ...
    /**
     * @param SwiftMailer_InputByteStream $inputStream
     * @param SwiftMailer_Message         $mimeEntity
     */
    protected function toSMimeByteStream(SwiftMailer_InputByteStream $inputStream, SwiftMailer_Message $message)
    {
        ...
        if (null !== $this->signCertificate && null !== $this->encryptCert) {
            $temporaryStream = new SwiftMailer_ByteStream_TemporaryFileByteStream();
     ...
     * @param SwiftMailer_Message $message
     *
     * @return SwiftMailer_Message
     */
    protected function createMessage(SwiftMailer_Message $message)
    {
        $mimeEntity = new SwiftMailer_Message('', $message->getBody(), $message->getContentType(), $message->getCharset());
    ...
    /**
     * @param SwiftMailer_FileStream      $outputStream
     * @param SwiftMailer_InputByteStream $inputStream
     *
     * @throws SwiftMailer_IoException
     */
    protected function messageStreamToSignedByteStream(SwiftMailer_FileStream $outputStream, SwiftMailer_InputByteStream $inputStream)
    {
        $signedMessageStream = new SwiftMailer_ByteStream_TemporaryFileByteStream();
        ...
        if (!call_user_func_array('openssl_pkcs7_sign', $args)) {
            throw new SwiftMailer_IoException(sprintf('Failed to sign S/Mime message. Error: "%s".', openssl_error_string()));
        }
     ...
     * @throws SwiftMailer_IoException
     */
    protected function messageStreamToEncryptedByteStream(SwiftMailer_FileStream $outputStream, SwiftMailer_InputByteStream $is)
    {
        $encryptedMessageStream = new SwiftMailer_ByteStream_TemporaryFileByteStream();

        if (!openssl_pkcs7_encrypt($outputStream->getPath(), $encryptedMessageStream->getPath(), $this->encryptCert, array(), 0, $this->encryptCipher)) {
            throw new SwiftMailer_IoException(sprintf('Failed to encrypt S/Mime message. Error: "%s".', openssl_error_string()));
        }
    ...
    /**
     * @param SwiftMailer_OutputByteStream $fromStream
     * @param SwiftMailer_InputByteStream  $toStream
     */
    protected function copyFromOpenSSLOutput(SwiftMailer_OutputByteStream $fromStream, SwiftMailer_InputByteStream $toStream)
    {
        $bufferLength = 4096;
        $filteredStream = new SwiftMailer_ByteStream_TemporaryFileByteStream();
    ...
    /**
     * Merges an OutputByteStream to SwiftMailer_Message.
     *
     * @param SwiftMailer_OutputByteStream $fromStream
     * @param SwiftMailer_Message          $message
     */
    protected function streamToMime(SwiftMailer_OutputByteStream $fromStream, SwiftMailer_Message $message)
    {
        $bufferLength = 78;
        $headerData = '';
        ...
        if ('multipart/signed;' === substr($headers['content-type'], 0, 17)) {
            if (!preg_match('/boundary=("[^"]+"|(?:[^\s]+|$))/is', $headers['content-type'], $contentTypeData)) {
                throw new SwiftMailer_SwiftMailerException('Failed to find Boundary parameter');
            }
            ...
            if (null === $this->headerFactory) {
                $this->headerFactory = SwiftMailer_DependencyContainer::getInstance()->lookup('mime.headerfactory');
            }
