 ...
 */
class SwiftMailer_Signers_DKIMSigner implements SwiftMailer_Signers_HeaderSigner
{
    /**
     * PrivateKey.
     *
     ...
     *
     * @var SwiftMailer_Mime_Headers_ParameterizedHeader
     */
     ...
     *
     * @throws SwiftMailer_SwiftMailerException
     *
     * @return $this
     */
        ...
        switch ($hash) {
            ...
            case 'rsa-sha256':
                ...
                if (!defined('OPENSSL_ALGO_SHA256')) {
                    throw new SwiftMailer_SwiftMailerException('Unable to set sha256 as it is not supported by OpenSSL.');
                }
                break;
            default:
                throw new SwiftMailer_SwiftMailerException('Unable to set the hash algorithm, must be one of rsa-sha1 or rsa-sha256 (%s given).', $hash);
        }
     ...
     * @param bool $debug
     *
     * @return SwiftMailer_Signers_DKIMSigner
     */
    ...
    private function _getEncryptedHash()
    {
        ...
        if (!$pkeyId) {
            throw new SwiftMailer_SwiftMailerException('Unable to load DKIM Private Key ['.openssl_error_string().']');
        }
        if (openssl_sign($this->_headerCanonData, $signature, $pkeyId, $algorithm)) {
            return $signature;
        }
        throw new SwiftMailer_SwiftMailerException('Unable to sign DKIM Hash ['.openssl_error_string().']');
    }
}
