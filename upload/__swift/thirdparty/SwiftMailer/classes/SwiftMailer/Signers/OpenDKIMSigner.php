 ...
 */
class SwiftMailer_Signers_OpenDKIMSigner extends SwiftMailer_Signers_DKIMSigner
{
    ...
    public function __construct($privateKey, $domainName, $selector)
    {
        if (!extension_loaded('opendkim')) {
            throw new SwiftMailer_SwiftMailerException('php-opendkim extension not found');
        }
    ...
    {
        $header = new SwiftMailer_Mime_Headers_OpenDKIMHeader('DKIM-Signature');
        $headerVal = $this->_dkimHandler->getSignatureHeader();
        if (!$headerVal) {
            throw new SwiftMailer_SwiftMailerException('OpenDKIM Error: '.$this->_dkimHandler->getError());
        }
    ...
    public function setHeaders(SwiftMailer_Mime_HeaderSet $headers)
    {
        ...
        if (!is_numeric($this->_signatureTimestamp)) {
            ...
            if (!OpenDKIM::setOption(OpenDKIM::OPTS_FIXEDTIME, $this->_signatureTimestamp)) {
                throw new SwiftMailer_SwiftMailerException('Unable to force signature timestamp ['.openssl_error_string().']');
            }
        }
