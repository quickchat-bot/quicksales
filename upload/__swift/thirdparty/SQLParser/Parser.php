...
class SQL_Parser
{
    ...
    static protected $_extendedOrderByFunctions = array(
        'customfield',
    );

    static protected $_specialGroupByFunctions = array(
        'x',
        'y',
    );
...
*/
    ...
    public function getParamsFunction(&$values, &$types, &$opts, $i = 0)
    {
        ...
        while ($open_braces > 0) {
            ...
            if (!empty($_tokenText) && isset($this->functions[$_tokenText])) { // FIXME: names like 'Password' or 'User' do not work (reserved?)
                $this->getTok();
                if ($this->token != '(') {
                    $this->raiseError('Expected "("');
                }
         ...
         && $this->token != ')') {
            switch ($this->token) {
                ...
                case 'order':
                    ...
                    $this->getTok();
                    while ($this->token == 'ident' || $this->token == 'text_val' || in_array($this->token, self::$_extendedOrderByFunctions)) {
                        $arg = $this->lexer->tokText;
                        if ($this->isFunc()) {
                            $col = $this->parseFunctionOpts();
                            if ($col !== false) {
                                $this->lexer->pushBack();
                            }
                        } else {
                ...
                case 'multigroup':
                    ...
                    while ($this->token == 'ident' || $this->token == 'text_val' || $this->token == ':' || $this->token == '(' || $this->token == ')' || in_array($this->token, self::$_extendedGroupByFunctions)) {
                        $arg = $this->lexer->tokText;
                        ...
                        if (in_array($_argComparison, self::$_specialGroupByFunctions)) {
                            $_options = array(); // Reset the options
                        ...
                        /**
                         * ---------------------------------------------
                         * Added support for functions - Andriy Lesyuk
                         * ---------------------------------------------
                         */
                        ...
                        if ($this->isFunc()) {
                            $result = $this->parseFunctionOpts();
                            if ($result !== false) {
                                $_options = array_merge($_options, $result);
                                $this->lexer->pushBack();
                                $col = '_function';
                            }
                        } else {
                        ...
                        // Support closure of X(...) and Y(...)
                        if ($this->token == ')' && isset($_options['function'])) {
                            $this->getTok();
                        }
